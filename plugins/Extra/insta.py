...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 [@sanctuary_stardust]")
        else:
            dump_file=await message.reply_video(url,caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 [@sanctuary_stardust]")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @sanctuary_stardust")
                   else:
                       dump_file=await message.reply_video(content_value, caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @sanctuary_stardust")
               except:
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @sanctuary_stardust")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @sanctuary_stardust")
                  except:
          ...
          #     await message.reply(tracemsg)
            ##optinal 
            await message.reply(f"400: Sorry, Unable To Find It  try another or report it  to @sanctuary_stardust")
            ...
            if 'downfile' in locals():
                os.remove(downfile)
            await message.reply("<a href='https://t.me/sanctuary_stardust'>ECHO</a>")
