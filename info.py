...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002172427490 -1002090397274 -1002242734668').split()]  # Channel id for auto indexing 'Current Database | MT 2.O | Old Database' (make sure bot is admin)
...
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002611085705 -1001992624088 -1002136991674 -1002806563649').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
VERIFIED_LOG = int(environ.get('VERIFIED_LOG', '-'))  # Log channel id (make sure bot is admin)
